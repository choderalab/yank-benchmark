...
def boostrap_mue(x, y, b, nreplicates=1000):
    ...
    for replicate in range(nreplicates):
        ...
        replicate_rmse[replicate] = mue(x[bootstrap_indices], y[bootstrap_indices], b)
    # report 95% CI
...
for index in range(nplots):
    ...
    local_mue = mue(xdata, ydata, local_fit)
    # Calculate CI
    ci_min, ci_max = boostrap_mue(xdata, ydata, local_fit)
    ...
    axes.annotate("MUE: {0:3.2f} [{1:3.2f}, {2:3.2f}] kcal/mol".format(local_mue, ci_min, ci_max),
                  xy=(0.5, 1.05),
