...
input_file = 'input/c-Met_bmcl_neutral_fepplus.sdf'
...
input_ifs = oechem.oemolistream(input_file)
for index, mol in enumerate(input_ifs.GetOEGraphMols()):
    exp_fe[index] = float(oechem.OEGetSDData(mol, 'r_fepplus_exp_dG')) 
    fepplus_fe[index] = float(oechem.OEGetSDData(mol, 'r_fepplus_pred_dG')) 
    fepplus_error[index] = float(oechem.OEGetSDData(mol, 'r_fepplus_pred_dG_error')) 
input_ifs.close()
print(exp_fe)
print(fepplus_fe)
print(fepplus_error)
